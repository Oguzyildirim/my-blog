...
- You simply blocked your thread to wait the network request and multithreading is not a solution anymore because context switching costs a lot and all threads add additional resource. You can't have unlimited number of threads you can do better than that it is time to break to walls. It is time to introduce something new! Actually the idea is simple instead of scaling your application horizontally or adding new threads. You just need to run your application on a single thread and make the I/O calls asynchronous. How is that sound now? Because remember network request are endless roads you experience it and you don't want to wait this roads to finish to do your other jobs. Basically you create and a thing call callback and when the road finished you got an signal saying you  **HEYYYYYY!!**, your request is here and you can proceed.
...
- The idea is simple DO NOT block the IO so we have APIs for developing event driven HTTP servers. I can't compare these tools, frameworks or whatever they are but every platform has their APIs today i want to talk about handling IO and now you can go and find out what you are going to achieve with this APIs.
